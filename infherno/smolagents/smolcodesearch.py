...
def search_for_code_or_coding(fhir_attribute_path: str, search_term: str) -> Tuple[str, List[Dict]]:
    """
    Search for a code/coding in a FHIR ValueSet using the SNOMED CT or HL7 ValueSets in English. Returns the found candidates that match the search term.
    ...
    try:
        ...
        if not results:
            ...
            result_text = f'Found results for `{search_term}` in `{fhir_attribute_path}` truncated to max. of {MAX_SEARCH_RESULTS} results.'
            result_output = []
...
def create_codesearch_agent(model) -> ToolCallingAgent:
    ...
    return ToolCallingAgent(
        ...
        description="Searches for codes and codings in FHIR ValueSets (SNOMED CT or HL7 ValueSets) for a certain search term and for a certain FHIR path (e.g. Condition.code). You need to explain that you are looking for a term and code for a certain FHIR property (e.g. Condition.code). The search term is the formal, English term to search for in the ValueSet (string-matching) in SNOMED CT or HL7 ValueSets. The FHIR path is the FHIR attribute path to search in (e.g., \"Condition.code\").",
        verbosity_level=2
    )
